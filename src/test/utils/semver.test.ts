...
describe('semver', () => {
  ...
  test.concurrent('semver', () => {
    ...
    expect(() => {Semver.fromString(strings.invalid)}).toThrowError()
    expect(() => {Semver.fromString(strings.empty)}).toThrowError()
    expect(() => {Semver.fromString(strings.undef)}).toThrowError()
  })
})
